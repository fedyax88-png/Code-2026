# =====================================================================================
# Z3.py — ИСПРАВЛЕННАЯ ВЕРСИЯ
#
# СПИСОК ВНЕСЁННЫХ ИСПРАВЛЕНИЙ (см. подробности в чате):
#   1. QTimer с интервалом 0 мс менял CPU в бесконечный спин -> заменён на реальный
#      интервал ~7 мс (144 Гц) с PreciseTimer.
#   2. config_manager.load_config() читал файл с диска в каждом кадре (в таймере,
#      в paintEvent, в обработчиках жестов, даже внутри цикла по кнопкам) ->
#      добавлено кэширование конфига (get_cached_config) с TTL 250 мс и функцией
#      принудительной инвалидации (invalidate_config_cache), которую можно вызывать
#      сразу после сохранения настроек.
#   3. Устранён двойной вызов self._execute_hardware_press_async(...) за один кадр
#      в on_face_gestures_received (эмуляция клавиш срабатывала дважды).
#   4. ctypes.Structure классы (MOUSEINPUT_SYS, KEYBDINPUT_SYS, ...) пересоздавались
#      на каждый вызов _execute_hardware_press_async -> вынесены на уровень модуля,
#      создаются один раз.
#   5. self.lbl_cursor.show()/hide() вызывались каждый тик без проверки текущего
#      состояния -> теперь вызываются только при реальной смене видимости.
#   6. Небезопасный доступ к self.mod_mouse.toolbar_window без hasattr -> добавлена
#      защита через getattr, как и для advanced_panel_window.
#   7. Голые "except:" заменены на "except Exception:".
#   8. Убран задвоенный .connect() на кнопке "Назад" (страница профилей).
#   9. os.kill(pid, SIGTERM) в closeEvent заменён на штатный QApplication.quit(),
#      чтобы Qt мог корректно завершить очереди событий и потоки.
#  10. Разовые импорты (ctypes, time, threading, QMetaObject, QCursor и т.д.),
#      которые раньше делались внутри "горячих" методов на каждый кадр, подняты
#      в начало файла.
# =====================================================================================
...
import config_manager  # Подключаем менеджер сохранений
...
# Импортируем новые страницы меню навигации
...
# =====================================================================================
# ИСПРАВЛЕНИЕ #2: Кэш конфигурации.
# Раньше config_manager.load_config() читал файл с диска на каждый кадр таймера/камеры
# (потенциально сотни-тысячи раз в секунду). Теперь конфиг перечитывается максимум раз
# в CONFIG_CACHE_TTL секунд, а при реальном изменении настроек можно вызвать
# invalidate_config_cache(), чтобы получить свежие данные немедленно.
# =====================================================================================
CONFIG_CACHE_TTL = 0.25  # секунд; перечитываем файл максимум 4 раза в секунду
...
def invalidate_config_cache():
    """Вызывать сразу после сохранения настроек, чтобы не ждать истечения TTL."""
    _config_cache["ts"] = 0.0


# =====================================================================================
# Структура WinAPI для сверхбыстрого извлечения точки из ядра Windows
# =====================================================================================
...
# =====================================================================================
# ИСПРАВЛЕНИЕ #4: структуры и хелперы SendInput вынесены на уровень модуля.
# Раньше эти классы объявлялись заново внутри _execute_hardware_press_async,
# который вызывается на каждый кадр распознавания жестов — это лишние аллокации
# и работа интерпретатора на пустом месте. Теперь объявляются один раз при импорте.
# =====================================================================================
_LONG = ctypes.c_long
...
# Константы абсолютного хардверного приоритета
...
class VirtualCursorOverlay(QWidget):
    """Кастомный геймерский оверлей: лазерный микроблок летит через кнопки без микро-зависаний на 144 Гц"""

    def __init__(self, parent=None):
        super().__init__(parent)
        # Независимые сквозные флаги окна, всегда висящего на самом верху графических слоев
        self.setWindowFlags(
        ...
        # Растягиваем оверлей на весь экран монитора
        ...
        # Запрашиваем точный масштаб системы Windows (например, 200% = 2.0)
        try:
            hdc = ctypes.windll.user32.GetDC(0)
            self.dpi_scale = ctypes.windll.gdi32.GetDeviceCaps(hdc, 88) / 96.0  # 88 - LOGPIXELSX
            ctypes.windll.user32.ReleaseDC(0, hdc)
        except Exception:
            self.dpi_scale = 1.0

        # 🔥 АППАРАТНЫЙ НЕОНОВЫЙ МИКРО-ВИДЖЕТ КУРСOРА
        self.lbl_cursor = QLabel(self)
        self.lbl_cursor.setFixedSize(16, 16)  # Маленький геймерский размер
        self.lbl_cursor.setStyleSheet("""
            QLabel {
                background-color: #a347ff;
                border: 2px solid #ffffff;
                border-radius: 8px; /* Делает квадрат идеальным круглым лазером */
            }
        """)
        # Отслеживаем текущую видимость лейбла, чтобы не дёргать show()/hide()
        # без необходимости на каждый тик таймера (ИСПРАВЛЕНИЕ #5)
        self._cursor_visible_state = None

        # -----------------------------------------------------------------------
        # ИСПРАВЛЕНИЕ #1: раньше таймер запускался с интервалом 0 мс, из-за чего
        # он срабатывал настолько часто, насколько успевал event loop — это
        # безусловно грузило одно ядро CPU на 100% и НЕ давало никакого прироста
        # плавности сверх реальной частоты обновления экрана. Теперь интервал
        # выставлен в ~1000/144 ≈ 7 мс с PreciseTimer, что реально соответствует
        # 144 Гц и не пожирает процессор впустую.
        # -----------------------------------------------------------------------
        ...
        # Низкоуровневое WinAPI пробитие приоритета окон поверх Direct3D кадров игры
        try:
            hwnd = int(self.winId())
    ...
    def sync_cursor_position_hardware_144hz(self):
        """Опрашивает истинные координаты Windows и двигает лейбл на частоте экрана 144 Гц"""
        # ИСПРАВЛЕНИЕ #2: используем кэшированный конфиг вместо чтения файла на каждый тик
        ...
        if show_custom_cursor:
            # ИСПРАВЛЕНИЕ #5: не дёргаем show() повторно, если лейбл уже виден
            ...
            if ctypes.windll.user32.GetCursorPos(ctypes.byref(pt)):
                ...
                # 🔥 АППАРАТНЫЙ СДВИГ СЛОЯ (БЕЗ ТОРМОЗОВ ТЯЖЁЛОГО PAINTEVENT)
                self.lbl_cursor.move(int(self.v_x), int(self.v_y))

                # Запускаем перерисовку легкого кольца dwell-клик зарядки только если мышь замерла
                if 0.0 < self.progress < 1.0:
                    self.update()
        else:
            # ИСПРАВЛЕНИЕ #5: не дёргаем hide() повторно, если лейбл уже скрыт
    ...
    def update_cursor_data(self, progress, virtual_x, virtual_y):
        """Метод-мост: синхронизирует прогресс dwell-клика для круга зарядки"""
        self.progress = progress

    def paintEvent(self, event):
        """Отрисовывает круг зарядки только вокруг лазерного кружка в моменты фиксации взгляда"""
        # ИСПРАВЛЕНИЕ #2: используем кэшированный конфиг вместо чтения файла на каждую перерисовку
        config = get_cached_config()
        current_prof = config.get("current_profile", "Default Profile")
        ...
        if show_click_circle and 0.0 < self.progress < 1.0:
            ...
            # Привязываем кольцо к центру нашего аппаратного лазерного виджета (16x16)
            cx = int(self.v_x) + 8
            cy = int(self.v_y) + 8
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Новые окна меню
        self.page_nav_profiles = ProfilesPage("СТРАРИЦА КОНФИГУРАЦИИ ПРОФИЛЕЙ", self)
        ...
        self.content_stack.addWidget(self.page_main)               # Индекс 0
        self.content_stack.addWidget(self.page_settings)           # Index 1
        self.content_stack.addWidget(self.page_voice_settings)     # Index 2
        self.content_stack.addWidget(self.page_mouse_settings)     # Index 3
        self.content_stack.addWidget(self.page_keyboard_settings)  # Index 4
        self.content_stack.addWidget(self.page_nav_profiles)       # Index 5
        self.content_stack.addWidget(self.page_nav_settings)       # Index 6
        self.content_stack.addWidget(self.page_nav_about)          # Index 7
        ...
        # СОЗДАЕМ И РАЗВЕРТЫВАЕМ КРУГОВОЙ ОВЕРЛЕЙ ЗАРЯДКИ КЛИКА НА ВЕСЬ ЭКРАН МОНИТОРА
    ...
    def setup_main_page_grid(self):
        ...
        # Связываем сигналы переключения окон для кнопок внутри 4-х модулей
        ...
        # Связываем сигналы левого меню навигации
    ...
    def on_face_coordinates_received(self, nx, ny):
        """ЦЕНТРАЛЬНЫЙ ДИСПЕТЧЕР УПРАВЛЕНИЯ: связывает ИИ-кадры с физическим курсором Windows"""
        # ИСПРАВЛЕНИЕ #2 + #10: конфиг кэшируется, импорты подняты в начало файла
        config = get_cached_config()
        current_prof = config.get("current_profile", "Default Profile")
        mouse_conf = config["profiles"][current_prof]["mouse"]

        # ПРОВЕРКА 1: Работает ли global рубильник модуля мыши?
        ...
        # =========================================================================
        # ⚡ АВТОНОМНАЯ ИИ-КЛАТЧ СИСТЕМА ДЛЯ РОДНОЙ КНОПКИ RST / F10 (ИЗОЛИРОВАННАЯ)
        # =========================================================================
        is_f10_active_now = False
        if hasattr(self, 'active_pressed_keys'):
            # Считываем статус горения кнопки F10/RST из ОЗУ, взведённый эмулятором ниже
            is_f10_active_now = self.active_pressed_keys.get("f10", False)

        # Инициализируем внутренний флаг готовности к обнулению нового нуля головы
        if not hasattr(self, '_f10_clutch_ready_for_reset'):
            self._f10_clutch_ready_for_reset = False

        if is_f10_active_now:
            # ЭТАП 1: ЗАМОРОЗКА. Пока бровь поднята, полностью блокируем передачу координат.
            self._f10_clutch_ready_for_reset = True

            # Мягко обновляем данные оверлея неонового курсора, чтобы он не пропадал, и выходим
            ...
            # ЭТАП 2: СБРОС И ОБНУЛЕНИЕ. Лицо расслабилось, бровь опустилась (выход из жеста)
            if self._f10_clutch_ready_for_reset:
                # 1. Вычисляем точные координаты геометрического центра экрана
                scr_w = self.mouse_controller.screen_width
                scr_h = self.mouse_controller.screen_height
                cx = int(scr_w // 2)
                cy = int(scr_h // 2)

                # 2. Аппаратно перемещаем реальный курсор Windows строго в середину монитора
                try:
                    ctypes.windll.user32.SetCursorPos(cx, cy)
                except Exception:
                    pass

                # 3. Вызываем родной метод reset контроллера
                self.mouse_controller.reset()

                # 4. 🔥 СВЕРХГЛУБОКАЯ СИНХРОНИЗАЦИЯ И СБРОС АВТОКЛИКЕРА ТУЛБАРА
                ...
                # Принудительно гасим зажимы мыши в ОС
                try:
        ...
        # Получаем параметры бегунков из меню настроек мыши
        ...
        # Проверяем положение мыши относительно окон программы
        pos = QCursor.pos()
        is_over_main_ui = self.geometry().contains(pos)

        # Проверяем, находится ли мышь физически над окном Advanced Panel
        ...
        # ИСПРАВЛЕНИЕ #6: защита через getattr, как и для advanced_panel_window,
        # чтобы не словить AttributeError, если toolbar_window ещё не создан
        ...
        # ПРОВЕРКА 2: Включена ли кнопка "Движение"?
        ...
        # =========================================================================
        # РЕЖИМ 1: ADVANCED PANEL ОТКРЫТА (ОНА СЕЙЧАС ЕДИНСТВЕННЫЙ ХОЗЯИН СИСТЕМЫ)
        # =========================================================================
        ...
        # =========================================================================
        # РЕЖИМ 2: ADVANCED PANEL ЗАКРЫТА (СТАНДАРТНЫЙ ТУЛБАР РАБОТАЕТ КАК ОБЫЧНО)
        # =========================================================================
        ...
        # Полностью восстанавливаем ИИ-мост для Set 1, чтобы проценты и лампы ожили
    ...
    def showEvent(self, event):
        """Интегрируем кнопки Назад прямо в верхние углы новых окон настроек при первом показе"""
        ...
        if not hasattr(self, '_video_back_btn_added'):
            ...
            header_layout_vid = QHBoxLayout(); header_layout_vid.addStretch(); header_layout_vid.addWidget(back_to_main_vid)
        ...
        if not hasattr(self, '_voice_back_btn_added'):
            ...
            header_layout_voice = QHBoxLayout(); header_layout_voice.addStretch(); header_layout_voice.addWidget(back_to_main_voice)
        ...
        if not hasattr(self, '_mouse_back_btn_added'):
            ...
            header_layout_mouse = QHBoxLayout(); header_layout_mouse.addStretch(); header_layout_mouse.addWidget(back_to_main_mouse)
            self.page_mouse_settings.layout().insertLayout(0, header_layout_mouse)
            self._mouse_back_btn_added = True

        # =========================================================================
        # СТРОГО ПО ТЗ: ТОЧНАЯ СБОРКА ЕДИНОЙ СТРОКИ НАВИГАЦИИ КЛАВИАТУРЫ (SET 4 / Set 5)
        # =========================================================================
        if not hasattr(self, '_kb_back_btn_added'):
            header_layout_kb = QHBoxLayout()
            header_layout_kb.setContentsMargins(0, 0, 0, 0)
            header_layout_kb.setSpacing(6)  # Микрозазор для левых кнопок навигации

            # 1. Добавляем кнопку «Настройки №2» строго с левой стороны ряда
            ...
            # 2. Добавляем кнопку «Назад к блокам» сразу вслед за ней в один ряд (изначально скрыта)
            ...
            # Логика асинхронного управления стэком многостраничности в kb_settings_main
            def open_set5_page_logic():
                if hasattr(self.page_keyboard_settings, 'set_active_page_index'):
                    self.page_keyboard_settings.set_active_page_index(1)  # Включаем Set 5 холст
                    self.btn_main_set5.setVisible(False)
                    self.btn_main_set5_back.setVisible(True)

            def close_set5_page_logic():
                if hasattr(self.page_keyboard_settings, 'set_active_page_index'):
                    self.page_keyboard_settings.set_active_page_index(0)  # Возвращаем блоки 1-4
            ...
            # 3. Пружина отталкивает левое меню влево, а крестик Назад — строго в правый край
            header_layout_kb.addStretch()

            # 4. Создаем оригинальную кнопку «✕ Назад» на её исходной высоте и законном месте
            ...
            # При клике наружу на крестик — всегда сбрасываем вид на блоки 1-4 и уходим на главный экран
            ...
            # Врезаем готовую строку на самый верх страницы
        ...
        if not hasattr(self, '_nav_profiles_back_added'):
            btn = QPushButton("✕ Назад"); btn.setFixedSize(90, 28); btn.setStyleSheet(NAV_BACK_STYLE)
            # ИСПРАВЛЕНИЕ #8: убран задвоенный .connect() (раньше строка была продублирована,
            # из-за чего setCurrentIndex(0) вызывался дважды подряд на каждый клик)
            btn.clicked.connect(lambda: self.content_stack.setCurrentIndex(0))
            lay = QHBoxLayout(); lay.addStretch(); lay.addWidget(btn)
    ...
    def on_face_gestures_received(self, gestures):
        """ЦЕНТРАЛЬНЫЙ ДИСПЕТЧЕР ЖЕСТОВ МИМИКИ ЛИЦА ИЗ КЛАВИАТУРЫ SET 2"""
        # ИСПРАВЛЕНИЕ #2: кэшированный конфиг вместо чтения файла на каждый кадр камеры
        config = get_cached_config()
        current_prof = config.get("current_profile", "Default Profile")
        kb_conf = config["profiles"][current_prof].get("kb", {})

        # Живая неоновая подсветка кнопок вынесена в самый верх
        live_thresholds = {}
        if hasattr(self, 'page_keyboard_settings') and self.page_keyboard_settings:
            block1 = getattr(self.page_keyboard_settings, 'block1', None)
        ...
        # =========================================================================
        #   ВЫСОКОСКОРОСТНОЙ ПИКСЕЛЬНЫЙ ДВИЖОК СКРОЛЛА «СК» ОТ ЦЕНТРА КНОПКИ
        # =========================================================================
        ...
        if is_gesture_mouse_allowed:
            ...
            # СКВОЗНОЙ ИИ-ПОИСК: Напрямую заходим в модуль клавиатуры и опрашиваем живую сетку его менеджера оверлеев
            try:
                manager_obj = None
            ...
            # Редкие одиночные уведомления в ОЗУ-мониторе (срабатывают строго 1 раз при клике)
            ...
            if is_overlay_scroll_active and scroll_btn_obj:
                # Намертво берем статичные координаты центра наэкранной кнопки за абсолютный ноль
                btn_geo = scroll_btn_obj.geometry()
                center_zero_y = float(btn_geo.y() + (btn_geo.height() / 2.0))

                # Считываем текущие пиксельные координаты реальной мыши Windows
                ...
                # Считаем пиксельное расстояние (дельту) от мышки до центра кнопки
                pixel_delta = current_mouse_y - self._blind_scroll_start_y
                abs_dy = abs(pixel_delta)

                # Градация скоростей тиков колесика мыши Windows каждые 100 пикселей отклонения от кнопки
                if abs_dy > 20:
                    if abs_dy <= 100:
                        interval_sec = 1.0      # Скорость 1 (от 20 до 100 пикселей): 1 тик в секунду
                    elif abs_dy <= 200:
                        interval_sec = 0.5      # Скорость 2 (от 100 до 200 пикселей): 2 тика в секунду
                    elif abs_dy <= 300:
                        interval_sec = 0.33     # Скорость 3 (от 200 до 300 пикселей): 3 тика в секунду
                    else:
                        interval_sec = 0.01     # Скорость 4: ТУРБО-РЕЖИМ (свыше 300 пикселей)

                    now_t = time.time()
                    if now_t - getattr(self, '_last_blind_scroll_time', 0.0) >= interval_sec:
                        try:
                            # Чистая, бесшумная Win32-эмуляция вращения колесика: вверх (120) или вниз (-120)
                            s_dir = 120 if pixel_delta < 0 else -120
                            ctypes.windll.user32.mouse_event(0x0800, 0, 0, s_dir, 0)  # 0x0800 - WHEEL
                        except Exception:
                            pass
        ...
        # ИСПРАВЛЕНИЕ #3: раньше _execute_hardware_press_async(...) вызывался ДВАЖДЫ за
        # один кадр (один раз до блока скролла и ещё раз здесь) — эмуляция клавиш и кликов
        # срабатывала вдвое чаще, что могло вызывать рассинхронизацию press/release.
        # Теперь вызов ровно один, в конце метода, когда все данные по жестам уже готовы.
    ...
    def _execute_hardware_press_async(self, gestures, bindings, thresholds, kb_conf, head_y):
        """ Фоновый эмулятор: обрабатывает рубильники (ПОЛНОЕ ИСПРАВЛЕНИЕ 4K И ТУЛБАРА) """
        # ИСПРАВЛЕНИЕ #4: ctypes-структуры и хелперы SendInput теперь объявлены один раз
        # на уровне модуля (см. начало файла) — здесь они больше не пересоздаются на
        # каждый вызов, что убирает лишние аллокации на каждый кадр распознавания жестов.
        ...
        # ЖЕЛЕЗОБЕТОННЫЙ ПЕРЕХВАТ КЛАTЧА RST (ОБНУЛЕНИЕ КУРСOРА)
        ...
        for gesture_id, key_name in bindings.items():
            ...
            # Извлекаем текущие логические координаты курсора для проверки геометрии PyQt6
            ...
            if is_triggered:
                if not self.active_pressed_keys[sys_key]:
                    if sys_key == "app":
                            ...
                            if hasattr(self, 'overlay_manager') and self.overlay_manager:
                                for btn in self.overlay_manager.active_buttons:
                                    if btn and btn.key_name == sys_key and btn.geometry().contains(logical_pos):
                                        # ИСПРАВЛЕНИЕ #2: кэшированный конфиг вместо чтения файла
                                        # внутри цикла по кнопкам (раньше читалось по разу на кнопку)
                        ...
                        # ОТПРАВКА АППАРАТНОГО НАЖАТИЯ ЧЕРЕЗ SENDINPUT БЕЗ СДВИГА СЕТКИ
                        if sys_key in ["lmb", "rmb", "mmb"]:
                            ...
                            scan = KEY_SCANCODES.get(sys_key, 0x01)
                ...
                if self.active_pressed_keys[sys_key]:
                    if sys_key == "app":
                        ...
                        # УМНЫЙ ИИ-ФИЛЬТР ТРИГГЕРНОГО РЕЖИМА НА РАССЛАБЛЕНИЕ ЛИЦА
                        is_target_toggle_mode = False
                        try:
                            if hasattr(self, 'overlay_manager') and self.overlay_manager:
                                for btn in self.overlay_manager.active_buttons:
                                    if btn and btn.key_name == sys_key and btn.geometry().contains(logical_pos):
                                        # ИСПРАВЛЕНИЕ #2: кэшированный конфиг
                                        cfg = get_cached_config()
                                        prof = cfg.get("current_profile", "Default Profile")
                                        if cfg["profiles"][prof].get("kb", {}).get("overlay_modes", {}).get(sys_key) == "toggle_click":
                                            is_target_toggle_mode = True
                        ...
                        # ОТПРАВКА АППАРАТНОГО ОТЖАТИЯ ЧЕРЕЗ SENDINPUT С ЧИСТЫМИ ОТСТУПАМИ КЛАССА
                        if sys_key in ["lmb", "rmb", "mmb"]:
    ...
    def closeEvent(self, event):
        """Корректное завершение работы программы и выгрузка ИИ-модулей из памяти"""
        if hasattr(self, 'mod_video') and self.mod_video and getattr(self.mod_video, 'camera_thread', None):
            try:
        ...
        # -----------------------------------------------------------------------
        # ИСПРАВЛЕНИЕ #9: раньше здесь стоял os.kill(os.getpid(), signal.SIGTERM),
        # что жёстко убивало процесс, минуя штатное завершение Qt (события,
        # деструкторы виджетов, отложенная очистка потоков). Теперь используется
        # штатный QApplication.quit(), а os._exit оставлен только как аварийный
        # fallback на случай, если какой-то не-daemon поток всё же не даст
        # процессу закрыться в разумное время.
        # -----------------------------------------------------------------------
        ...
        # Аварийный предохранитель: если через 3 секунды процесс всё ещё жив
        # (например, завис не-daemon поток) — гарантированно завершаем его.
