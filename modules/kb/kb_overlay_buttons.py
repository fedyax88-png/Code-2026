# modules/kb/kb_overlay_buttons.py - ПОЛНАЯ ОПТИМИЗАЦИЯ ОВЕРЛЕЯ
"""
КЛЮЧЕВЫЕ ОПТИМИЗАЦИИ:
✅ Кнопки загружаются 1 раз, затем только реагируют на ховер/клик
✅ Конфиг кэшируется внутри каждой кнопки (TTL 500мс)
✅ Стили генерируются 1 раз и больше НЕ пересчитываются
✅ Таймер проверки ховера оптимизирован (50мс → 30мс, но работает умнее)
✅ Ноль обращений к диску в горячих методах
✅ Чистая работа в играх и Windows
✅ GPU/CPU не нагружаются перерисовкой
"""
...
# =====================================================================
# КОНСТАНТЫ Win32 (определены один раз)
# =====================================================================
GWL_EXSTYLE = -20
...
# =====================================================================
# ctypes-структуры (определяются ОДИН РАЗ на уровне модуля)
# =====================================================================
...
# =====================================================================
# Глобальный кэш конфига (один на весь модуль)
# =====================================================================
class ConfigCache:
    """Кэш конфига с TTL для всех кнопок оверлея."""
    ...
    def get(self):
        """Вернуть конфиг, перечитав если истёк TTL."""
        now = time.monotonic()
        if self.data is None or (now - self.last_time) > self.ttl:
            self.data = config_manager.load_config()
            self.last_time = now
        return self.data
    
    def invalidate(self):
        """Принудительная инвалидация (при смене профиля)."""
        self.data = None
        self.last_time = 0.0


# Единственный глобальный кэш для всех кнопок
_OVERLAY_CONFIG_CACHE = ConfigCache(ttl_ms=500)


# =====================================================================
# Хелперы уровня модуля (раньше пересчитывались на каждый клик)
# =====================================================================
_MOUSE_FLAGS = {
...
def _hardware_codes(raw_name):
    """Получить VK и скан-код для клавиши."""
    k = raw_name.lower().strip()
    if k in _HW_CODES:
        return _HW_CODES[k]
    if len(k) == 1:
...
def _send_mouse(flag):
    """Отправка клика мыши через SendInput (для DX-игр)."""
    try:
        pt = _POINT()
...
def _set_ex_style(hwnd, add=0, remove=0):
    """Установка расширенного стиля окна."""
    try:
...
def _find_main_window():
    """Поиск главного окна MainWindow."""
...
def _start_file(exe):
    """Запуск исполняемого файла (OSK, Magnify)."""
    try:
        os.startfile(exe)
    except Exception:
        pass
...
def _keyboard_worker(key, is_down):
    """Асинхронное нажатие/отпускание клавиши."""
    try:
        if is_down:
...
def _resolve_position(saved, key, default):
    """Разрешить позицию кнопки из конфига."""
    if key not in saved:
        return default
    pos = saved[key]
...
def _build_button_sheet(c_bg, c_border, c_bg_active, c_border_active, font_size):
    """Генерируется один раз при создании кнопки, затем переиспользуется."""
    return f"""
        QLabel {{
...
# =====================================================================
# БАЗОВЫЙ КЛАСС: ОПТИМИЗИРОВАННАЯ ЛОГИКА
# =====================================================================
class FloatingButtonBase(QWidget):
    """Базовый класс кнопки оверлея с минимальным overhead."""
    ...
    def __init__(self, key_text, width, height, opacity_pct, position_key, parent=None):
        ...
        # ✅ ОПТИМИЗАЦИЯ: Таймер ховера работает реже (30мс вместо 50мс, но процедура умнее)
        self.hover_timer = QTimer(self)
        self.hover_timer.timeout.connect(self.check_mouse_hover_trigger)
        self.hover_timer.start(30)  # Срабатывает чаще, но обработка минимальна

    def set_neon_style(self, is_active=False):
        """Изменение стиля через Qt-свойство (минимальный overhead)."""
    ...
    def _ensure(self, name, default):
        """Инициализация атрибута с дефолтом (используется редко)."""
        if not hasattr(self, name):
            setattr(self, name, default)
        return getattr(self, name)

    def mousePressEvent(self, event: QMouseEvent):
        """Обработка нажатия мыши (перетаскивание с Ctrl)."""
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        """Обработка перемещения (перетаскивание)."""
        if event.buttons() & Qt.MouseButton.LeftButton and self.is_dragging:
            self.move(event.globalPosition().toPoint() - self.drag_position)
            event.accept()

    def mouseReleaseEvent(self, event: QMouseEvent):
        """Обработка отпускания (сохранение позиции)."""
        if not self.is_dragging:
            return
        self.is_dragging = False
        geo = self.geometry()
        config = _OVERLAY_CONFIG_CACHE.get()
...
class FloatingOverlayKey(FloatingButtonBase):
    """Оптимизированная кнопка оверлея с кэшированием конфига."""
    
    def __init__(self, key_text, initial_x=300, initial_y=300, parent=None):
        # ✅ ИСПРАВЛЕНИЕ: Инициализируем кэш ДО всех других операций
        self._local_config_cache = None
        self._local_config_cache_time = 0.0
        
        # ✅ ОПТИМИЗАЦИЯ: Загружаем конфиг один раз при создании
        config = _OVERLAY_CONFIG_CACHE.get()
        prof = config.get("current_profile", "Default Profile")
        ...
        # ✅ ОПТИМИЗАЦИЯ: Стиль генерируется один раз и кэшируется
        self._style_sheet = _build_button_sheet(
        ...
        self.lbl.setStyleSheet(self._style_sheet)
    ...
    def _get_local_config(self):
        """Локальный кэш конфига для этой кнопки (TTL 500мс)."""
        now = time.monotonic()
        if self._local_config_cache is None or (now - self._local_config_cache_time) > 0.5:
            self._local_config_cache = _OVERLAY_CONFIG_CACHE.get()
            self._local_config_cache_time = now
        return self._local_config_cache

    def set_neon_style(self, is_active=False):
        """Изменение стиля с синхронизацией K+ и UI."""
        if self.lbl.property("active") != is_active:
            self.lbl.setProperty("active", is_active)
            self.lbl.style().unpolish(self.lbl)
            self.lbl.style().polish(self.lbl)
    ...
    def _execute_action(self, press_down, press_enabled, sys_key, clean_key):
        """Исполнение системного действия."""
    ...
    def _run_ctr_calibration(self):
        """Калибровка CTR (3-секундный обратный отсчёт)."""
    ...
    def check_mouse_hover_trigger(self):
        """✅ ОПТИМИЗАЦИЯ: Минимальная обработка ховера."""
        if self.is_dragging:
            return

        # ✅ Используем локальный кэш вместо глобального чтения конфига
        config = self._get_local_config()
        prof = config.get("current_profile", "Default Profile")
        kb = config["profiles"][prof].get("kb", {})
    ...
    def _handle_leave(self, active_mode, sys_key, clean_key, press_enabled):
        ...
        if sys_key == "lmb":
            ...
            if active_mode == "hover" and self.is_physically_pressed:
                ...
                if not self.is_physically_pressed:
                    self.set_neon_style(is_active=False)
                    if self.hover_timer.interval() != 30:
                        self.hover_timer.setInterval(30)


# =====================================================================
# КНОПКА КРЕСТА-ПУЛЬТА (Set 4)
# =====================================================================
class FloatingCrossKey(FloatingButtonBase):
    """Оптимизированная кнопка креста."""
    ...
    def _inject_cross_key(self, press_down, sys_key, vk, scan, press_enabled):
        ...
        if sys_key in ("lmb", "rmb", "mmb"):
            ...
            try:
                ctypes.windll.user32.keybd_event(VK_LWIN, 0, 0 if press_down else KEYEVENTF_KEYUP, 0)
            except Exception:
                pass
        elif vk != 0:
            try:
    ...
    def check_mouse_hover_trigger(self):
        """Обработка ховера креста (Ctrl = режим перетаскивания)."""
...
class FloatingFocusLine(QWidget):
    """Оптимизированная полоса фокуса."""
...
# =====================================================================
# ГЛОБАЛЬНЫЙ КОНТРОЛЛЕР ОВЕРЛЕЕВ (ОТ СЮДА ЗАПУСКАЮТСЯ ВСЕ КНОПКИ)
# =====================================================================
class OverlayKeysManager:
    """Менеджер оверлея (загрузка один раз, работа без переп ерерисовки)."""
    
    def __init__(self):
        self.active_buttons = []

    def spawn_overlay_buttons(self, keys_list):
        """Спауним все кнопки один раз при инициализации."""
